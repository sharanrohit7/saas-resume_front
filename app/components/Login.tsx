...
export default function LoginCard() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      await signInUser(signInData);
      router.push("/analysis"); // Redirect after successful login
    } catch (error) {
      console.error("❌ Google Login failed:", error);
      setError("Google login failed. Please try again.");
    }
  ...
  return (
    <div className="flex flex-col items-center justify-center space-y-4 py-8">
      <button
        onClick={handleGoogleLogin}
        className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-2xl shadow-md text-lg font-semibold transition-transform duration-200 transform hover:scale-105"
      >
        🚀 Get Started
      </button>
      ...
      <button
        onClick={handleGoogleLogin}
        className="bg-gray-900 hover:bg-gray-800 text-white px-6 py-3 rounded-2xl shadow-md text-lg font-semibold transition-transform duration-200 transform hover:scale-105"
      >
        🎉 Start Free
      </button>
