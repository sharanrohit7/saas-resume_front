...
import { FiUpload, FiAlertCircle, FiCheckCircle, FiLoader, FiAward, FiBook, FiCode, FiBarChart2, FiEdit2, FiStar, FiChevronRight, FiTrendingUp, FiClock } from "react-icons/fi";
...
export default function ResumeAnalysis() {
    ...
    useEffect(() => {
        ...
        const fetchScoreHistory = async () => {
            try {
                const response = await fetch("/api/ats-scores");
    ...
    const analyzeResume = async () => {
        ...
        try {
            const response = await fetch("http://localhost:8091/resume/analyze", {
                method: "POST",
                headers: {
            ...
            {result?.plan === "BASIC" && (
                <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-3 rounded-lg mb-6 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span>Upgrade to Premium for detailed analysis, keyword optimization, and competitive insights</span>
                    </div>
                    <Link href="/plans" className="flex items-center gap-1 bg-white text-blue-600 px-3 py-1 rounded-md text-sm font-medium hover:bg-gray-100 transition-colors">
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="flex-1">
                    ...
                    <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 mb-6">
                        <h2 className="text-xl font-semibold mb-4 text-white">Analyze Your Resume</h2>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    />
                                    <button className="bg-gray-700 hover:bg-gray-600 px-4 rounded-lg border border-gray-600 flex items-center gap-2">
                                        <FiUpload /> Upload
                                    </button>
                                </div>
                            </div>
                        ...
                        <div className="mb-4">
                            <label className="block text-gray-400 mb-2">Company Name (Optional)</label>
                            <input
                                type="text"
                                value={companyName}
                        ...
                        <div className="mb-6">
                            <label className="block text-gray-400 mb-2">Job Description (Optional)</label>
                            <textarea
                                value={jobDesc}
                        ...
                        <button
                            ...
                            disabled={isLoading}
                            className={`flex items-center gap-2 px-6 py-3 rounded-lg font-medium ${isLoading
                    ...
                    {result && (
                        <div className="space-y-6">
                            {result.plan === "BASIC" ? (
                                ...
                                <AdvancedAnalysisView data={result.data as AdvancedAnalysisData} />
                            )}
                        </div>
                    )}
                </div>
...
function BasicAnalysisView({ data }: { data: BasicAnalysisData }) {
    return (
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
            <div className="mb-6">
                <h2 className="text-xl font-semibold mb-2 text-white">Basic Analysis</h2>
                <p className="text-gray-400 mb-4">Upgrade to Premium for detailed insights, keyword optimization, and competitive positioning</p>

                <div className="bg-gradient-to-r from-gray-700 to-gray-800 p-4 rounded-lg border border-gray-600 mb-6">
                    <h3 className="text-lg font-semibold mb-3 text-white flex items-center gap-2">
                        <FiStar className="text-yellow-400" /> What You're Missing in Premium
                    </h3>
                    ...
                    <Link href="/plans" className="mt-4 inline-block bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-md text-sm font-medium transition-colors">
                        Upgrade Now
                    </Link>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="bg-gray-700 rounded-lg p-4 border-l-4 border-blue-400">
                    ...
                    <p className="text-3xl font-bold text-blue-400">{data.score_breakdown.ats_score}%</p>
                </div>
                    ...
                    <div className="bg-gray-700 p-4 rounded-lg">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {data.gap_analysis.skills.missing_soft.map((skill, i) => (
                                <span key={i} className="bg-yellow-900/50 text-yellow-400 px-3 py-1 rounded-full text-sm">
                                    {skill}
                                </span>
                            ))}
                        </div>
                ...
                <div className="space-y-4">
                    {data.recommendations.priority_order.map((priority) => (
                        <div key={priority} className="bg-gray-700 p-4 rounded-lg">
                            <h4 className="capitalize font-medium text-white mb-2">{priority} Priority</h4>
...
//   }
  ...
  // Helper Components
  function AnalysisCard({ title, value, description, icon, borderColor }: {
    title: string;
    value: string;
    description: string;
  ...
  function AdvancedAnalysisView({ data }: { data: AdvancedAnalysisData }) {
    return (
      <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            <div className="bg-blue-900/30 text-blue-400 px-4 py-2 rounded-lg">
              ...
              <div className="text-2xl font-bold">{data.score_breakdown.ats_score}%</div>
            </div>
            <div className="bg-purple-900/30 text-purple-400 px-4 py-2 rounded-lg">
              <div className="text-xs text-purple-300">Title Match</div>
              <div className="text-2xl font-bold">{data.metadata.job_title_analysis.similarity_score}%</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-700 rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <KeywordMatchBox 
              type="exact" 
              items={data.deep_analysis.keyword_breakdown.resume_matches.exact} 
              count={data.deep_analysis.keyword_breakdown.resume_matches.exact.length}
            />
            <KeywordMatchBox 
              type="partial" 
              items={data.deep_analysis.keyword_breakdown.resume_matches.partial} 
              count={data.deep_analysis.keyword_breakdown.resume_matches.partial.length}
            />
            <KeywordMatchBox 
              type="missing" 
              items={data.deep_analysis.keyword_breakdown.resume_matches.missing} 
              count={data.deep_analysis.keyword_breakdown.resume_matches.missing.length}
            />
          </div>
          ...
          <div className="space-y-4">
            {Object.entries(data.deep_analysis.keyword_breakdown.density_analysis.current_distribution).map(([keyword, density]) => (
              <div key={keyword}>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-gray-300">{keyword}</span>
                  <span className={parseFloat(density) === 0 ? "text-red-400" : "text-blue-400"}>
                    {density}
                  </span>
                </div>
                <div className="w-full bg-gray-600 rounded-full h-2">
                  <div 
                    className={`h-2 rounded-full ${
                      parseFloat(density) === 0 ? "bg-red-400" :
                      parseFloat(density) < 2 ? "bg-yellow-400" : "bg-green-400"
                    }`}
            ...
            <p className="text-gray-500 text-xs mt-2">
              Ideal density range: {data.deep_analysis.keyword_breakdown.density_analysis.ideal_range}
            </p>
          </div>
        </div>
          ...
          <div className="bg-gray-700 rounded-lg p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {data.gap_analysis.skills.missing_soft.map((skill, i) => (
                    <span key={i} className="bg-yellow-900/30 text-yellow-400 px-3 py-1 rounded-full text-sm">
                      {skill}
                    </span>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {data.gap_analysis.skills.partial_matches.map((skill, i) => (
                    <span key={i} className="bg-blue-900/30 text-blue-400 px-3 py-1 rounded-full text-sm">
                      {skill}
                    </span>
                  ))}
                </div>
        ...
        <div className="bg-gray-700 rounded-lg p-6">
          <h3 className="text-lg font-semibold mb-4 text-white flex items-center gap-2">
            <FiCheckCircle className="text-green-400" /> Optimization Recommendations
          </h3>
          <div className="space-y-6">
            {data.recommendations.priority_order.map(priority => (
              <div key={priority} className="space-y-2">
                <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                  ...
                  priority === "high" ? "bg-yellow-900/30 text-yellow-400" :
              ...
              <div className="space-y-4">
                {data.deep_analysis.achievement_analysis.weak_statements.map((statement, i) => (
                  <div key={i} className="border-l-2 border-blue-400 pl-4">
                    <p className="text-gray-400 text-sm mb-1">Original:</p>
                    <p className="text-gray-300 mb-2 text-sm">{statement.original}</p>
                    <p className="text-gray-400 text-sm mb-1">Improved:</p>
                    <p className="text-green-400 mb-1 text-sm">{statement.improved}</p>
                    <p className="text-gray-500 text-xs">{statement.improvement_reason}</p>
                  </div>
                ))}
              </div>
  ...
  // Helper Components
  function ScorePill({ title, score, ideal }: { title: string; score: number; ideal: number }) {
          ...
          <svg className="w-full h-full" viewBox="0 0 36 36">
            ...
            <path
              d="M18 2.0845
                ...
                a 15.9155 15.9155 0 0 1 0 -31.831"
              fill="none"
              stroke={percentage >= 75 ? "#48BB78" : percentage >= 50 ? "#ECC94B" : "#F56565"}
              strokeWidth="3"
  ...
  function KeywordMatchBox({ type, items, count }: { type: string; items: string[]; count: number }) {
    const colorMap = {
      exact: { bg: "bg-green-900/20", text: "text-green-400", border: "border-green-400/30" },
      partial: { bg: "bg-yellow-900/20", text: "text-yellow-400", border: "border-yellow-400/30" },
      missing: { bg: "bg-red-900/20", text: "text-red-400", border: "border-red-400/30" }
    };
    ...
    return (
      <div className={`p-4 rounded-lg border ${border} ${bg}`}>
        <div className="flex justify-between items-center mb-2">
          <h4 className={`text-sm font-medium capitalize ${text}`}>{type} Matches</h4>
          <span className={`text-xs px-2 py-1 rounded-full ${bg} ${text}`}>{count}</span>
        </div>
        <div className="flex flex-wrap gap-2">
          {items.slice(0, 5).map((item, i) => (
            <span key={i} className={`text-xs px-2 py-1 rounded-full ${bg} ${text} border ${border}`}>
              {item}
            </span>
          ))}
          {items.length === 0 && (
  ...
  function MatchBox({ type, items, color }: {
    type: string;
    items: string[];
    ...
    return (
      <div className="bg-gray-700 p-4 rounded-lg">
        ...
        <div className="flex flex-wrap gap-2">
          {items.slice(0, 5).map((item, i) => (
            <span key={i} className={`px-3 py-1 rounded-full text-sm border ${colorClasses[color]}`}>
              {item}
            </span>
          ))}
          {items.length === 0 && (
...
function UpgradeCard() {
    return (
        <div className="bg-gradient-to-br from-blue-900/50 to-purple-900/50 rounded-xl border border-blue-700/30 p-6">
            ...
            <Link href="/plans" className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white py-2 rounded-lg font-medium text-center block transition-colors">
                Upgrade Now
            </Link>
        </div>
    );
